# Architecture Decision Records

Los ADRs son inmutables una vez reemplazados: una decisión nueva debe enlazar y marcar la anterior como superseded.

- [ADR-0001 — Monolito modular hexagonal](0001-modular-monolith-hexagonal.md)
- [ADR-0002 — Baseline de versiones y ARM64](0002-version-baseline-arm64.md)
- [ADR-0003 — Integraciones sólo mediante puertos y fakes](0003-integration-ports-and-fakes.md)
- [ADR-0004 — Topología Docker con ingreso y redes segmentadas](0004-docker-network-topology.md)
- [ADR-0005 — Flyway y namespaces acotados](0005-flyway-schema-boundaries.md)
- [ADR-0006 — Identidad, sesiones y bootstrap atomico](0006-identity-sessions-and-bootstrap.md)
- [ADR-0007 — Credenciales por usuario y salida a proveedores](0007-provider-credentials-and-egress.md)
- [ADR-0008 — Ciclo de generación persistente y SSE](0008-persistent-generation-and-sse.md)
